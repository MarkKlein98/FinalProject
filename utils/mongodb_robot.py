...
def main():
    ...
    while True:
        ...
        if choice == "1":
            ...
            collection_name = input("Enter collection name: ")
            see_document_ids = input('Do you wish to see all documents? (y/n): ')
            if see_document_ids == 'y':
                display_all_documents_in_a_collection(db, collection_name)
            else:
                pass
            document_id = input("Enter document ID: ")
            display_specific_document(db, collection_name, document_id)
            key = input("Enter key: ")
...
def display_specific_document(db, collection_name, document_id):
    collection = db[collection_name]
    documents = collection.find()
    for document in documents:
        for item in document.values():
            if str(item) == document_id:
                print('')
                print('[[Document Keys]]\n-----------------')
                for key in document:
                    print(f'[{key}]', end=' ')
